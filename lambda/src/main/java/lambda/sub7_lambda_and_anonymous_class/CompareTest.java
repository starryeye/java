...
public class CompareTest {
     ...
     *      익명 클래스 내부에서 언체크 예외를 발생시키면 외부로 쭉 전파가 된다.
     *      익명 클래스 내부에서 체크 예외를 발생시키려면, 해당 추상 메서드에 throws 절이 있어야 컴파일 에러 없이 던질 수 있으며, 발생하면 외부로 쭉 전파 된다.
     *
     *
     * 람다(익명 함수)
     *      이름이 없다.
     ...
     *      람다 외부의 변수에 접근할 수 있지만, final 혹은 effectively final 변수여야 한다. (변수 캡처링 가능)
     *      람다 내부에서 언체크 예외를 발생시키면 외부로 쭉 전파가 된다.
     *      람다 내부에서 체크 예외를 발생시키려면, 해당 함수형 인터페이스 추상 메서드에 throws 절이 있어야 컴파일 에러 없이 던질 수 있으며, 발생하면 외부로 쭉 전파 된다.
     *          참고, Java 가 제공하는 대부분의 함수형 인터페이스에는 throws 절이 없어서..
     *          람다 내부에서 체크 예외를 다뤄야 한다면 try-catch 로 예외를 잡아서 언체크 예외로 변경을 해주거나 예외 해결을 해야한다.
     *
     * 참고
