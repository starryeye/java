package dev.practice.advance.nio;
...
//                    log.info("server wait accept..");
     ...
     * todo, 위의 문제 상황이 accept 처리 스레드가 하나여서(ㄱ)인지.. 근본적인 문제(ㄴ)여서인지.. 생각해볼 것..
     * (ㄱ) -> 멀티 스레드로 해결할 수 있다..?
     * (ㄴ) -> accept 를 처리하는 스레드가 여러개라도 동일하게 성능 문제가 있을 수 있다. (C10K.. 인듯)
     * (ㄴ) -> 이를 해결하기 위해서는 이벤트로 처리해야한다. (비동기, callback)
     *
     * <참고>
