...
public interface Executor1 {
     ...
     *      3. Runnable 인터페이스를 사용하면, 작업 결과에 관심있는 외부 스레드에서 반환 값, 체크 예외를 받아 처리하는 개념이 없음
     *
     * 위 3가지 문제의 해결을 해야한다.
     *
     * "Executor framework 를 사용하면" 해결
     * thread pool 도입
     ...
     *          -> 2번 해결
     * Callable, Future 도입
     *      - 작업 결과에 관심있는 외부 스레드에서 반환 값, 체크 예외를 받아 처리하도록 해주는 Callable, Future
     *          -> 3번 해결
     *
     * 참고
