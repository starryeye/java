...
public class Future1 {
     ...
     *      파라미터
     *          true : Future 를 CANCELLED 상태로 변경, 작업이 실행 중이면 작업 실행 스레드에 인터럽트를 호출해서 작업을 중단시킨다.
     ...
     *      반환값은 작업의 결과이다.
     *      get() 을 호출했을 때, Future 의 상태가 FAILED 이면 예외(ExecutionException)가 발생
     *      예외
     ...
     *                                  내부에 작업 실행 스레드에서 발생한 예외가 포함되어있다. (ExecutionException::getCause())
     *
     *
     ...
     *      예외
     *          InterruptedException : get() 메서드 호출 스레드가 대기 중에 인터럽트 발생한 경우
     *          ExecutionException : 작업 실행 스레드에서 작업 중 예외가 발생한 경우, get() 메서드를 호출한 스레드에서 발생하는 예외이다.
     *                                  내부에 작업 실행 스레드에서 발생한 예외가 포함되어있다.
     *          TimeoutException : 파라미터로 전달한 일정 시간내에 작업이 완료되지 못하면, get() 메서드를 호출한 스레드에서 발생하는 예외이다.
     */
}
