...
public class BoundedBufferQueueV2 implements BoundedBufferQueue {
     ...
     * 다음과 같은 문제가 있다.
     *      1. 버퍼가 가득차있고 대기 스레드 큐에 생산자 스레드들이 많은 경우..
     *          소비자 스레드가 데이터를 소비하고 생산자 스레드들 중 하나가 깨워져서 생산을 정상적으로 했는데
     *          생산 후 notify() 에 의해 생산자 스레드가 깨워지는 문제가 존재한다.(데이터는 이미 가득 차있는데 생산자 스레드가 깨워져서 아무것도 못하고 다시 대기큐에 진입)
     *      2. 버퍼가 비워져있고 대기 스레드 큐에 소비자 스레드들이 많은 경우..
     *          생산자 스레드가 데이터를 생산하고 소비자자 스레드들 중 하나가 깨워져서 데이터 소비를 정상적으로 했는데
     *          소비 후 notify() 에 의해 소비자 스레드가 깨워지는 문제가 존재한다.(데이터는 이미 없어졌는데 소비자 스레드가 깨워져서 아무것도 못하고 다시 대기큐에 진입)
     * 1, 2 번 문제를 위해
     * 스레드 대기 큐를 2개로 분리해서 소비자 스레드는 생산자 스레드만 깨우고 생산자 스레드는 소비자 스레드만 깨울 수 있도록 한다.
     *
     * ReentrantLock..
    ...
    private final Condition producerCond = lock.newCondition(); // 생산자 스레드 대기 큐
    private final Condition consumerCond = lock.newCondition(); // 소비자 스레드 대기 큐
