...
public class BoundedBufferQueueV1 implements BoundedBufferQueue {
    ...
    /**
     * 사전 지식.
     *      모든 객체에는 모니터 락과 대기 스레드 큐가 존재한다.
     *      모니터 락
     *          synchronized 키워드를 사용할 경우 임계영역에 진입하는 스레드가 획득해야하는 락이다.
     *          1개의 스레드만 진입 가능, 임계영역을 벗어나면 획득했던 락을 반납한다.
     *          락 획득을 대기하는 스레드는 BLOCKED 상태로 차단된다. (cpu 소모 X)
     *      대기 스레드 큐
     *          임계 영역 내에서 Object::wait() 를 호출한 스레드가 진입하여 대기하는 공간이다. (RUNNALBE -> WAITING, 락 반납)
     *          임계 영역 내에서 Object::notify() 를 호출하면 대기 스레드 큐에 존재하는 스레드 중 하나가 깨워진다. (WAITING -> BLOCKED)
     *              notify() 를 호출한 스레드가 임계영역을 벗어나서 락을 반납하면, 대기 스레드 큐에서 깨워진 스레드가 락을 획득한다. (BLOCKED -> RUNNABLE)
     *
     * 한정된 버퍼 문제를 해결하기 위해..
