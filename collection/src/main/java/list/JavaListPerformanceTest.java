...
public class JavaListPerformanceTest {
    ...
    public static void main(String[] args) {
         ...
         */
        System.out.println("\n== ArrayList :: add ==");
        addFirst(new ArrayList<>(), n); // 위치 찾기 O(1), element 추가 O(n)
        addMid(new ArrayList<>(), n); // 위치 찾기 O(1), element 추가 O(n)
        addLast(new ArrayList<>(), n); // 위치 찾기 O(1), element 추가 O(1)     (last index 에 추가하면 그 이후 index 들이 없어서 +1 shift 할게 없으므로 O(1) 이다.)

        System.out.println("\n== ArrayList :: access ==");
        List<Integer> arrayListForAccess = getInitializedArrayList(n);
        ...
        System.out.println("\n== ArrayList :: search ==");
        List<Integer> arrayListForSearch = getInitializedArrayList(n);
         ...
         */
        System.out.println("\n== LinkedList :: add ==");
        addFirst(new LinkedList<>(), n); // 위치 찾기 O(1), element 추가 O(1)       (first, last index 의 경우에는 내부적으로 doubly linked list 구조로 head, tail 참조 값을 가지기 때문에 O(1) 로 접근 가능)
        addMid(new LinkedList<>(), n); // 위치 찾기 O(n), element 추가 O(1)
        addLast(new LinkedList<>(), n); // 위치 찾기 O(1), element 추가 O(1)        (first, last index 의 경우에는 내부적으로 doubly linked list 구조로 head, tail 참조 값을 가지기 때문에 O(1) 로 접근 가능)

        System.out.println("\n== LinkedList :: access ==");
        List<Integer> linkedListForAccess = getInitializedLinkedList(n);
        ...
        System.out.println("\n== LinkedList :: search ==");
        List<Integer> linkedListForSearch = getInitializedLinkedList(n);
    ...
    private static List<Integer> getInitializedArrayList(int n) {
    ...
    private static List<Integer> getInitializedLinkedList(int n) {
    ...
    private static void addFirst(List<Integer> list, int n) {
    ...
    private static void addMid(List<Integer> list, int n) {
    ...
    private static void addLast(List<Integer> list, int n) {
    ...
    private static void access(List<Integer> list, int n, int index) {
    ...
    private static void search(List<Integer> list, int n, int value) {
